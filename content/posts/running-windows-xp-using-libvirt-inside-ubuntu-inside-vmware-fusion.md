...
So I'm investigating running Windows XP inside [`libvirt`][libvirt] in Ubuntu on my MacBook for continuous integration testing
purposes with [Jenkins][jenkins]. I tried for many hours to get it working but Windows would not boot up or it would get stuck
on the `NTLDR cannot be found` issue. As it turns out, I did not enable "Enable hypervisor applications in this
virtual machine". I was under the assumption that I would not need to check that box as I wrongly assumed [`qemu`][qemu]
...
pedigrees, I tried installing Windows XP with [`virt-manager`][virt-manager] and all of them failed to bootup with various disk image
...
bootup and only using `qemu` without [`kvm`][kvm] will not cut it for booting Windows XP inside libvirt inside Ubuntu inside
